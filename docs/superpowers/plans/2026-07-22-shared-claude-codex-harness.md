...
- Create `common/.harness/features/dev-sidebar/workflow.md`: shared build, deployment, and verification facts.
...
- Create `common/.harness/bin/check-branches.sh`: shared real-repository branch gate.
...
- [ ] Add tests for exact four-column parsing, feature-specific verifier discovery, shared branch checks, and parity rejecting one-sided or synchronized adapter drift.
- [ ] Add tests for verifier `RESULT PASS`, strict `RESULT INCOMPLETE`, and demo-only `RESULT EXPLORATION` behavior.
...
- [ ] Return normalized `key=value` contract fields, target branch, workflow path, and a SHA-256 hash over the public fact files.
- [ ] Implement deterministic demo assertions with strict PASS/FAIL/INCOMPLETE semantics.
- [ ] Compare each adapter with the canonical resolver contract, reject copied public facts, and validate both context references.
