# review 包 bbdc50d6..f91f54d3

## commit 列表

```
f91f54d fix(session): support shallow quality checkouts
...
```
 tests/test-session-path.sh | 249 ++++++++++++++++++++++++---------------------
...
+  invoke p s HARNESS_STATE_ROOT="$TMP_TEST/harness/state" XDG_RUNTIME_DIR="$TMP_TEST/xdg" TMPDIR="$TMP_TEST/tmp"
...
+  tmp_before=$(find "$TMP_TEST/tmp" -printf '%P|%y|%m|%s\n' | LC_ALL=C sort)
+  invoke p s -u HARNESS_STATE_ROOT XDG_RUNTIME_DIR="$TMP_TEST/xdg" TMPDIR="$TMP_TEST/tmp"
+  expect 'root XDG' 0 "$TMP_TEST/xdg/aosp-harness-$(id -u)/p/s"$'\n' ''
+  [[ $tmp_before == "$(find "$TMP_TEST/tmp" -printf '%P|%y|%m|%s\n' | LC_ALL=C sort)" ]] || fail 'root XDG: lower priority changed'
+  invoke p s -u HARNESS_STATE_ROOT -u XDG_RUNTIME_DIR TMPDIR="$TMP_TEST/tmp"
...
+  invoke "$default_project" s -u HARNESS_STATE_ROOT -u XDG_RUNTIME_DIR -u TMPDIR
+  expect 'root default' 0 "$DEFAULT_PROJECT_PATH/s"$'\n' ''
+  invoke p s HARNESS_STATE_ROOT="$TMP_TEST/logical/state"
...
+  invoke p s PYTHONPATH="$TMP_TEST/fault" HARNESS_STATE_ROOT="$TMP_TEST/post-mkdir/state"
...
+    invoke "$project" "$session" HARNESS_STATE_ROOT="$TMP_TEST/isolation/state"
...
+    invoke project session HARNESS_STATE_ROOT="$root"
...
@@ -208,49 +219,59 @@ cp "$AUDIT/names-before" "$AUDIT/names-expected"
