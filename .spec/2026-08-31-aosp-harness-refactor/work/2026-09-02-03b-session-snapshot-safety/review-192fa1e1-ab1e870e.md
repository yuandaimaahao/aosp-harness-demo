# review 包 192fa1e1..ab1e870e

## commit 列表

```
ab1e870 test(session): add snapshot safety matrix
```

## diff --stat

```
 tests/test-session-snapshot.sh | 192 +++++++++++++++++++++++++++++++++++++++++
...
```diff
diff --git a/tests/test-session-snapshot.sh b/tests/test-session-snapshot.sh
new file mode 100755
index 0000000..a92a61d
--- /dev/null
+++ b/tests/test-session-snapshot.sh
@@ -0,0 +1,192 @@
+#!/usr/bin/env bash
+set -u
+case ${1-} in
