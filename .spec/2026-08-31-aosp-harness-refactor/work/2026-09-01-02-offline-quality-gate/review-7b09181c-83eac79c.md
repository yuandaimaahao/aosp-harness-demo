# review 包 7b09181c..83eac79c

## commit 列表

```
83eac79 test(quality): harden docs contract oracle
```

## diff --stat

```
 tests/test-quality-gate.sh | 56 +++++++++++++++++++++++++---------------------
...
index 1601d5f..ee7bad7 100755
--- a/tests/test-quality-gate.sh
+++ b/tests/test-quality-gate.sh
@@ -103,46 +103,52 @@ for mutation in "${mutations[@]}"; do cp "$baseline" "$static_fixture/scripts/sh
...
+def check_workflow(raw):
+ assert hashlib.sha256(raw).hexdigest()=='fe394030c377c2b4949bbfa127fa4957209ff42c078682f5ec42e14778b154bc'; w=raw.decode(); lines=w.splitlines()
+ assert lines[1]=='on: [push, pull_request, workflow_dispatch]'
+ assert lines[2:9]==['jobs:','  quality:','    runs-on: ubuntu-24.04','    steps:','      - uses: actions/checkout@v4','      - name: Install pinned quality tools','        run: |']
+ commands=('set -euo pipefail','root="$RUNNER_TEMP/aosp-harness-quality"; bin="$root/bin"','mkdir -p "$root/downloads" "$bin"','curl -fsSL -o "$root/downloads/shellcheck.tar.xz" https://github.com/koalaman/shellcheck/releases/download/v0.11.0/shellcheck-v0.11.0.linux.x86_64.tar.xz',"printf '%s  %s\\n' 8c3be12b05d5c177a04c29e3c78ce89ac86f1595681cab149b65b97c4e227198 \"$root/downloads/shellcheck.tar.xz\" | sha256sum -c -",'tar -xJf "$root/downloads/shellcheck.tar.xz" -C "$root"','install -m 0755 "$root/shellcheck-v0.11.0/shellcheck" "$bin/shellcheck"','curl -fsSL -o "$root/downloads/shfmt" https://github.com/mvdan/sh/releases/download/v3.14.0/shfmt_v3.14.0_linux_amd64',"printf '%s  %s\\n' fe42021c7272ef2d67ea36cbc3031683c625d0badec733ef3a57b567246a0b66 \"$root/downloads/shfmt\" | sha256sum -c -",'install -m 0755 "$root/downloads/shfmt" "$bin/shfmt"','curl -fsSL -o "$root/downloads/gitleaks.tar.gz" https://github.com/gitleaks/gitleaks/releases/download/v8.30.1/gitleaks_8.30.1_linux_x64.tar.gz',"printf '%s  %s\\n' 551f6fc83ea457d62a0d98237cbad105af8d557003051f41f3e7ca7b3f2470eb \"$root/downloads/gitleaks.tar.gz\" | sha256sum -c -",'tar -xzf "$root/downloads/gitleaks.tar.gz" -C "$root"','install -m 0755 "$root/gitleaks" "$bin/gitleaks"',"printf '%s\\n' \"$bin\" >>\"$GITHUB_PATH\"")
+ assert lines[9:24]==['          '+command for command in commands]
+ assert lines[24:]==['      - name: Quality gate','        run: ./scripts/check.sh --ci'] and w.count('./scripts/check.sh --ci')==1
+def check_coverage(c):
+ rows=[]; header=separator=0
+ assert not re.search(r'(?i)(?:(?:行|分支|覆盖率|\b(?:line|branch|coverage)\b).*\d+(?:\.\d+)?%?|\d+(?:\.\d+)?%?.*(?:行|分支|覆盖率|\b(?:line|branch|coverage)\b))',c)
+ for raw in c.splitlines():
+  if not raw.strip(): continue
+  indent=len(raw)-len(raw.lstrip(' ')); assert indent<=3
+  line=raw[indent:]; assert line.startswith('|') and line.endswith('|')
+  cells=[cell.strip() for cell in line.strip('|').split('|')]
+  if cells==['Test','Specs/requirements','Protected behavior','Offline boundary','Status']: header+=1; continue
+  if cells==['---']*5: separator+=1; continue
+  assert len(cells)==5 and all(cells) and cells[4]=='active',cells
+  match=re.fullmatch(r'`(tests/test-[^`]+\.sh)`',cells[0]); assert match; rows.append(match.group(1))
+ expected={'tests/'+entry.name for entry in os.scandir('tests') if entry.is_file(follow_symlinks=False) and entry.name.startswith('test-') and entry.name.endswith('.sh')}
+ assert header==separator==1 and len(rows)==len(set(rows)) and set(rows)==expected,(rows,expected)
+w=Path(sys.argv[1]).read_bytes(); c=Path(sys.argv[2]).read_text(); check_workflow(w); check_coverage(c)
+rejects(check_workflow,w.replace(b'on: [',b'events: [',1)+b'note: "on: [push, pull_request, workflow_dispatch]"\n')
+data=next(line for line in c.splitlines() if line.startswith('| `tests/'))
+rejects(check_coverage,c+'   '+data+'\n'); rejects(check_coverage,c+'   '+data.replace('test-device-safety','test-unknown')+'\n')
+rejects(check_coverage,c.replace('Device targeting and legacy regressions','100% branch coverage'))
 PY
 }
