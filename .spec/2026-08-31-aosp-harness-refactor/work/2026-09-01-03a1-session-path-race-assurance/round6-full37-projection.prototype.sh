...
for layer in ("root", "project", "session"):
    for kind in ("safe", "unsafe", "disappear"):
...
for kind, include, expected, phase, made in managed_specs:
    for layer in ("root", "project", "session"):
...
for layer in ("root", "project", "session"): wrong_euid(layer)
...
expected_ids = [f"swap-{layer}-{kind}" for layer in layers for kind in ("safe-dir", "link", "file")]
expected_ids += [f"eexist-{layer}-{kind}" for layer in layers for kind in ("safe", "unsafe", "disappear")]
expected_ids += [f"{kind}-{layer}" for kind, *_ in managed_specs for layer in layers]
expected_ids += [f"wrong-euid-{layer}" for layer in layers] + ["real-eio"]
check(case_ids == expected_ids and len(set(case_ids)) == 37, "exact ordered unique case IDs")
