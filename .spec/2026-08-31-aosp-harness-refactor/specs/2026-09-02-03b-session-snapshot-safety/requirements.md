...
验收方式: 混合判定
已由用户确认: true
确认依据: 用户已授权后续按autopilot执行；PLAN v5.7把Python child信号清理归还03b并新增03b1顺序门；03a2启动门已满足，prototype 7ffc213补齐child latch、strict worker零state副作用、四态source完整inventory/export、held capture结构、静态攻击整树delta、managed disappearance/suffix、字节级read oracle与第二波并发完整winner指纹，固定格式实跑core199+base198=397/400且exact2 ShellCheck全绿；同commit的03b1完整active family固定格式274/400、227项调用/状态delta全绿
---
...
- [ ] capture/managed-EUID/managed-before-open/snapshot-EUID/snapshot-before-open/temp/publish及OS八个anchor在生产文本各精确一次且不读测试环境；write worker在任何`.snapshot-*` owned-temp前安装三个signal handler，finally只清owned temp，publish成功后不回滚winner；read无信号协议。
