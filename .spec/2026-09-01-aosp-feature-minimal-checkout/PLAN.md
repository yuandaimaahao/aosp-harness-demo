...
|---|---|---|---|---|
| 2026-09-01-00a-seed-contract-runtime | 固化 dispatcher、seed/evidence schema、canonical digest、state-dir 与 recoverable store/ref runtime | — | `./common/.harness/bin/feature-closure verify-seed common/tests/fixtures/aosp17-services/seed.golden.json` 输出 `SEED ABI PASS`；schema/path/digest/collision/ref fault matrix 精确 fail closed；预计非生成 diff ≤730 行 | ⬜ 未开始 |
| 2026-09-01-00b-environment-seed-probe | 消费 00a runtime，在真实 local/public source 上产出 seed 或 terminal report，并证明 source/network/build guard | 2026-09-01-00a-seed-contract-runtime | `preflight --descriptor FILE --state-dir ABS_DIR --out-ref REF` 对 real public seed stdout 仅为 `ENV PASS public_aosp17_cuttlefish`；随后 `verify-seed --ref ... --require-public-real` 退 0，control plane 才记录 `GATE CONTINUE_PUBLIC`；预计非生成 diff ≤630 行 | ⬜ 未开始 |
| 01-feature-lock-contract | 建立 feature 意图、platform SHA lock 和 closure proof v1 契约与 fail-closed 校验 | 2026-09-01-00b-environment-seed-probe | `./common/.harness/bin/feature-closure verify-lock common/tests/fixtures/closure/valid.json` 输出 `LOCK PASS`；每个角色无 witness、缺 SHA、非唯一 path 和占位 product 均被指定错误码拒绝 | ⬜ 未开始 |
