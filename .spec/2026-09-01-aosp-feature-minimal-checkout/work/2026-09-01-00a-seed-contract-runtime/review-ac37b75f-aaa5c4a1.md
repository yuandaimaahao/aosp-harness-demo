...
```
 ...
 common/tests/test_seed_contract_runtime.py              | 7 ++++++-
...
+def terminal():
+    runtime=__import__("seed_contract_runtime"); copy=__import__("copy"); golden=load_artifact(path=str(Path(__file__).parent/"fixtures/aosp17-services/seed.golden.json"),expected_kind="seed"); assert golden["evidence_class"]=="fixture_only" and golden["source_scope"]["role"]=="public_aosp17_cuttlefish" and runtime._object_bytes(golden)==canonical_bytes(value=golden)+b"\n" and domain_digest(domain_ascii=runtime._DOMAINS["seed"],value=golden)=="07ed847af333505342d14f49d28982f6bcc307984dbf928cd762b49c7872587f"
+    report={"schema_version":1,"kind":"terminal_report","request_digest":"0"*64,"primary_reason":"SOURCE_ROOT_UNAVAILABLE","failed_checks":["SOURCE_ROOT_UNAVAILABLE","TRACE_UNAVAILABLE"],"completed_observation_digests":{"source_state":None,"trace":"0"*64,"command_journal":None},"summary_lines":["source root unavailable"]}; assert runtime._validate_artifact(report)==runtime._DOMAINS["terminal_report"] and runtime._object_bytes(report)==canonical_bytes(value=report)+b"\n"; evidence(); seed()
+    for fn in (lambda x:x.update(primary_reason="TRACE_UNAVAILABLE"),lambda x:x.update(failed_checks=["TRACE_UNAVAILABLE","SOURCE_ROOT_UNAVAILABLE"]),lambda x:x.update(failed_checks=["SOURCE_ROOT_UNAVAILABLE","SOURCE_ROOT_UNAVAILABLE"]),lambda x:x["completed_observation_digests"].update(trace="bad"),lambda x:x["summary_lines"].append("2026-09-01 12:00:00Z"),lambda x:x.update(extra=None)): changed=copy.deepcopy(report); fn(changed); bad(lambda changed=changed:runtime._validate_artifact(changed),"DESCRIPTOR_SCHEMA_INVALID")
