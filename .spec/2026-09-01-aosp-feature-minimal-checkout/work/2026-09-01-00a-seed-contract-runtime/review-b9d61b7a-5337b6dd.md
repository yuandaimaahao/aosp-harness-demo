...
+    if v["entry_kind"] == "regular": _hex(v["content_sha256"]); v["symlink_target_sha256"] is None or _bad()
+    elif v["entry_kind"] == "symlink": _hex(v["symlink_target_sha256"]); v["content_sha256"] is None or _bad()
+    elif v["entry_kind"] == "missing": v["mode"] == 0 and v["content_sha256"] is None and v["symlink_target_sha256"] is None or _bad()
+    else: _bad()
+def _project(v):
...
+    if type(v) is not dict or type(v.get("schema_version")) is not int: _bad()
+    if v["schema_version"] != 1: raise ContractError("UNSUPPORTED_SCHEMA_VERSION")
...
+    req={"schema_version":1,"kind":"seed_request","source_root":os.getcwd(),"envsetup_relpath":"build/envsetup.sh","lunch":{"target":"a","product":"b","release":"c","variant":"d"},"source_scope":{"role":"local_lk7k_product","public_aosp_baseline":False,"vendor_context":True,"platform_family":"aosp-17"},"resource_minimums":{"available_bytes":0,"available_inodes":0,"effective_memory_bytes":0,"effective_cpus":0},"estimated_disk_upper_bound_bytes":0}
...
+        project_domain="aosp-harness/project-source-state/v1\0"; assert runtime._project(state["projects"][0]) == project_domain and domain_digest(domain_ascii=project_domain,value=state["projects"][0]) == hashlib.sha256(project_domain.encode()+canonical_bytes(value=state["projects"][0])).hexdigest(); link=Path(d)/"source-link"; link.symlink_to(d,target_is_directory=True)
+        for value, mutate in ((req,lambda x:x.update(schema_version=True)),(req,lambda x:x.update(unexpected=None)),(req,lambda x:x.pop("lunch")),(req,lambda x:x["lunch"].update(target=None)),(req,lambda x:x["source_scope"].update(public_aosp_baseline=0)),(req,lambda x:x["source_scope"].update(vendor_context=1)),(req,lambda x:x["resource_minimums"].update(available_bytes=True)),(req,lambda x:x["resource_minimums"].update(available_bytes=2**53)),(req,lambda x:x.update(source_root=os.getcwd()+"//")),(req,lambda x:x.update(source_root="//"+os.getcwd().lstrip("/"))),(req,lambda x:x.update(source_root=str(link))),(state,lambda x:x["projects"].append(x["projects"][0])),(state,lambda x:x["projects"][0]["entries"].append(x["projects"][0]["entries"][0])),(state,lambda x:x["projects"][0]["entries"][0].update(path_b64="?")),(trace,lambda x:x["records"][0].update(sequence=2**53)),(trace,lambda x:x["records"][0].update(result=-(2**53),errno="E")),(trace,lambda x:x["records"][0].update(classification="bad")),(trace,lambda x:x["records"][0].update(exec_argv_b64=None)),(journal,lambda x:x["records"].reverse()),(journal,lambda x:x["records"][0].pop("cwd_b64")),(journal,lambda x:x.update(records=[0])),(journal,lambda x:x.update(records=[[]])),(journal,lambda x:x.update(records=[None]))):
