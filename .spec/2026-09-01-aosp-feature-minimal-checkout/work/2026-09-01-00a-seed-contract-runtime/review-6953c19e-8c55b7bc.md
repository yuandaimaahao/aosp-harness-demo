# review 包 6953c19e..8c55b7bc

## commit 列表

```
8c55b7b feat(harness): add seed verification cli
```
...
+    for cmd,code in (((str(dispatch),),"ARGUMENT_ERROR"),((str(dispatch),"BAD!","--bad"),"INVALID_COMMAND"),((str(direct),),"ARGUMENT_ERROR"),((str(direct),str(golden),"--ref","x"),"ARGUMENT_ERROR"),((str(direct),"--artifact-store","x","--ref","y"),"ARGUMENT_ERROR"),((str(direct),"--wat","x"),"ARGUMENT_ERROR"),((str(direct),"--ref","x"),"ARGUMENT_ERROR"),((str(direct),"--ref","x","--ref","y","--artifact-store","z"),"ARGUMENT_ERROR"),((str(direct),"--artifact-store","z","--ref","x"),"ARGUMENT_ERROR"),((str(direct),"--ref","x","--artifact-store","z","--require-public-real","extra"),"ARGUMENT_ERROR"),((str(direct),"--ref","x","--artifact-store","z","--require-public-real","--require-public-real"),"ARGUMENT_ERROR")): check(cmd,code)
...
+        a=("--ref",str(ref),"--artifact-store",str(store)); tree=lambda:tuple(sorted((str(x.relative_to(state)),x.read_bytes() if x.is_file() else b"") for x in state.rglob("*"))); before=tree(); one=run(str(direct),*a); two=run(str(dispatch),"verify-seed",*a); exact(one,0,"SEED ABI PASS\n",""); exact(two,one.returncode,one.stdout,one.stderr); check((str(direct),*a,"--require-public-real"),"PUBLIC_SCOPE_REQUIRED"); check((str(dispatch),"verify-seed",*a,"--require-public-real"),"PUBLIC_SCOPE_REQUIRED"); assert tree()==before and seed["seed_content_digest"]==runtime.domain_digest(domain_ascii=runtime._DOMAINS["seed_content"],value=runtime._seed_parts(seed)[1]) and seed["seed_identity_digest"]==runtime.domain_digest(domain_ascii=runtime._DOMAINS["seed_identity"],value=runtime._seed_parts(seed)[2])
