...
- path/symlink/containment failure 在 state-dir 外创建的 filesystem entries = 0（≤ 0），验证: bash common/tests/test-seed-contract-runtime.sh --case path-confinement
...
| 集成 | no-follow path、forbidden roots、object idempotence/collision、lock/ref、evidence closure、fault matrix、temp/sentinel 不变量 | 临时目录 + Python subprocess/fcntl |
...
- [ ] 步骤 1: 写 `state-paths` case，覆盖 tilde/relative/missing/unwritable/symlink/containment、missing leaf、exact store/out-ref derivation 与 immutable exact keys。
...
- [ ] 步骤 4: 最小实现逐分量 directory-fd openat/mkdirat/O_NOFOLLOW 与 forbidden-root component containment。
