...
+        parent=state/"bad"; parent.symlink_to(root); regular=state/"refs/regular"; regular.touch(); assert runtime.validate_state_paths(state_dir=str(state),out_ref=str(regular),artifact_store=str(store),forbidden_roots=())["out_ref"]==str(regular); fifo=state/"refs/fifo"; os.mkfifo(fifo); bad(lambda:runtime.validate_state_paths(state_dir=str(state),out_ref=str(fifo),artifact_store=str(store),forbidden_roots=()),"OUT_REF_CONTRACT"); sock=state/"refs/sock"; q=__import__("socket").socket(__import__("socket").AF_UNIX); q.bind(str(sock)); bad(lambda:runtime.validate_state_paths(state_dir=str(state),out_ref=str(sock),artifact_store=str(store),forbidden_roots=()),"OUT_REF_CONTRACT"); q.close(); bad(lambda:runtime.validate_state_paths(state_dir=str(state),out_ref=str(regular.parent),artifact_store=str(store),forbidden_roots=()),"OUT_REF_CONTRACT"); sy=root/"store-link"; sy.mkdir(); (sy/"artifacts").symlink_to(root); bad(lambda:runtime.validate_state_paths(state_dir=str(sy),out_ref=None,artifact_store=str(sy/"artifacts/v1"),forbidden_roots=()),"STATE_DIR_CONTRACT")
+        bads += ((str(state),str(store),str(parent/"leaf"),(),"OUT_REF_CONTRACT"),)
+        race=root/"race"; race.mkdir(); artifact=race/"artifacts"; artifact.mkdir(); outside=root/"outside"; outside.mkdir(); old=runtime._same; hit=[False]; runtime._same=lambda fd,path:old(fd,path) if path!=str(artifact) or hit[0] else(hit.__setitem__(0,True) or os.rename(artifact,outside/"moved") or old(fd,path)); bad(lambda:runtime.validate_state_paths(state_dir=str(race),out_ref=None,artifact_store=str(race/"artifacts/v1"),forbidden_roots=()),"STATE_DIR_CONTRACT"); runtime._same=old; assert hit==[True] and not (outside/"moved/v1").exists(); blocked=root/"blocked"; blocked.mkdir(); blocked.chmod(0o500)
