...
```
 .../closure/v1/lib/seed_contract_runtime.py        | 59 ++++++++++++++++++++--
...
@@ -269,10 +272,60 @@ def publish_object(*, state_dir: str, artifact_store: str, object_kind: str, pay
...
+        return nodes,lock
+    except BlockingIOError: _drop(nodes+([(lock,"")] if lock is not None else [])); raise ContractError("REF_BUSY") from None
+    except (OSError,TypeError,ValueError): _drop(nodes+([(lock,"")] if lock is not None else [])); raise ContractError("OUT_REF_CONTRACT") from None
...
+    (object_kind,ref_kind) in (("seed","env_pass"),("terminal_report","terminal_report")) or (_ for _ in ()).throw(ContractError("ARGUMENT_ERROR")); domain=_validate_artifact(payload); payload["kind"]==object_kind or (_ for _ in ()).throw(ContractError("ARGUMENT_ERROR")); paths=validate_state_paths(state_dir=state_dir,out_ref=None,artifact_store=artifact_store,forbidden_roots=forbidden_roots); (os.path.isabs(out_ref) and out_ref==os.path.normpath(out_ref) and out_ref!=paths["state_dir"] and out_ref.startswith(paths["state_dir"]+"/") and os.path.basename(out_ref) not in ("",".","..") and not any(out_ref==x or out_ref.startswith(x+"/") for x in forbidden_roots)) or (_ for _ in ()).throw(ContractError("OUT_REF_CONTRACT")); nodes=[]; objects=[]; lock=refdir=None; temp=None; renamed=False
+    try:
+        nodes,lock=_lock_ref(os.path.dirname(out_ref),os.path.basename(out_ref)); parent=nodes[-1][0]; refdir=os.open(".",os.O_RDONLY|os.O_DIRECTORY|os.O_NOFOLLOW,dir_fd=parent); objects,_=_walk(paths["object_dir"]); objectfd=objects[-1][0]; _existing_ref(parent,os.path.basename(out_ref),objectfd); _evidence(objectfd,payload)
+        try:
+            for n in os.listdir(refdir):
+                if re.fullmatch(r"\."+re.escape(os.path.basename(out_ref))+r"\.tmp\.[0-9]+\.[0-9a-f]{16}",n): p=os.open(n,os.O_PATH|os.O_NOFOLLOW,dir_fd=refdir); stat=os.fstat(p); _close(p); __import__("stat").S_ISREG(stat.st_mode) and __import__("stat").S_IMODE(stat.st_mode)==0o600 and os.unlink(n,dir_fd=refdir)
+        except OSError: pass
